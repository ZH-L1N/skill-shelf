...
- Cloudflare Pages can silently stop deploying ("disconnected from your Git account" banner in the project) — pushes then never build; fix via the Git repository row's **Manage** (re-authorize the GitHub App), then push to confirm. Never trust "deployed" without curling the live site for a fingerprint of the new commit
- The site side of Pages verification: `curl -sL "https://skill-shelf.pages.dev/?cb=$(date +%s)"` and grep for something the new commit introduced (cache-buster included; the HTML is must-revalidate but be safe)
